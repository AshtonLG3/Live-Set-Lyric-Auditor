...
describe("vocal transcript quality gate", () => {
  it("rejects a LALAL stem transcript dominated by a short repeated phrase", () => {
  ...
  it("marks original audio fallback reports explicitly", () => {
    ...
    const fallback = buildFallbackQualityReport(selected, rejected, "the LALAL.AI stem repeated one phrase");

    expect(fallback.status).toBe("fallback_original");
    expect(fallback.fallbackUsed).toBe(true);
    expect(fallback.rejectedSource).toBe("lalalai");
